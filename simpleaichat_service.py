...
def q_and_a_with_tools(response_instructions:str = "You are an AI assistant. You will answer queries in a helpful manner", queries:list[str]=[]) -> None:
    ai = init_azure_openai()
    contextualized_queries = q_and_a(response_instructions= "You  are an AI assistant. You will respond with a rephrased prompt with additional context only if needed. Else respond with the original prompt", 
             queries=queries)
...
if __name__ == "__main__":
    ...
    #q_and_a_with_tools(response_instructions= "You  are an AI assistant. You will answer queries in one sentence", queries=["what is the maximum temperature in chennai, India today","who is kamala Harris","what is the maximum temperature in Watertown,Boston, USA today", "where was she born", "when was she born", "What are her educational qualifications", "for which football club does lionel messi play for now in 2023"])
